if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("bload's admin is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "bload's admin is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "BIG DICK FE v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- 3INCHES Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the 3INCHES support server.'}
